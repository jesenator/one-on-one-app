    ...
    <>
      {slotFilter && (
        <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-md px-4 py-3 mb-4">
          ...
          <Link
            href="/attendees"
            className="text-xs text-amber-600 font-medium bg-amber-100 rounded px-2 py-1 hover:bg-amber-200 transition"
          >
            Clear
          </Link>
        </div>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          autoFocus
          className="w-full border border-stone-200 rounded-md pl-10 pr-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-accent-500/20 focus:border-accent-500 transition"
        />
      </div>
      {filtered.length === 0 ? (
          ...
          {filtered.map((a) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="font-medium text-sm text-stone-900 group-hover:text-accent-600 transition">{a.name}</div>
                <div className="text-xs text-stone-400 truncate">{a.email}</div>
              </div>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-stone-300 group-hover:text-accent-500 transition shrink-0">
